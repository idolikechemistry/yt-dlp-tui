...
pub fn interactive_config_setup(config_path: &Path, mut config: Config) -> Result<()> {
    loop {
        ...
        let selection = Select::new("🛠️ dl-media 偏好設定引導 (請使用上下鍵選擇項目)：", options)
            .prompt()
        ...
        if selection.contains("📂") {
            ...
            let input_path = Text::new("📍 請拖入下載路徑：")
                .prompt()
                .unwrap_or_default();
            ...
            println!("\n💡 操作指引：");
            println!("  1. 系統即將為您開啟該設定資料夾。");
            println!("  2. 請將存放 cookie_site.txt 的資料夾「拖曳」進此終端機視窗中，並按下 Enter。\n");
...
pub fn handle_cookies(
    ...
    if target_file.exists() {
        ...
        // 沒有實體檔，但偵測到為受限內容
        println!("\n⚠️ 本影片為限制級/私有內容，需要登入權限才能下載。");
        println!("   未在設定夾偵測到 {} 的專屬 Cookie 檔案 ({})", site_target, expected_filename);
        ...
        if want_to_wait {
            ...
            if target_file.exists() {
                ...
                println!("⚠️ 仍未發現 Cookie 檔案，系統將嘗試以「無登入狀態」繼續（可能隨後失敗）。");
            }
        }
    }
...
pub fn wait_for_manual_cookie(
    ...
    open_folder(resolved_cookie_dir)?;
    println!("\n⏳ 請將您的限制破解 Cookie 檔案重新命名為「{}」並放入剛開啟的資料夾中...", expected_filename);
    ...
    if target_file.exists() {
        ...
        println!("⚠️ 仍未檢測到實體 Cookie 檔案，放棄此次重試。");
    }
